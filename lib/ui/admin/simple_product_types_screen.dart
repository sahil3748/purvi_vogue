...
class SimpleProductTypesScreen extends StatefulWidget {
  final String? preselectedSubcategoryId;
  final ProductTypeModel? editingProductType;

  const SimpleProductTypesScreen({
    super.key,
  ...
  @override
  State<SimpleProductTypesScreen> createState() =>
      _SimpleProductTypesScreenState();
}

class _SimpleProductTypesScreenState extends State<SimpleProductTypesScreen> {
  ...
  ProductTypeModel? _editingProductType;
  ...
  void _initializeForm() {
    if (widget.editingProductType != null) {
      _isEditing = true;
      _editingProductType = widget.editingProductType;
      _nameController.text = widget.editingProductType!.name;
      _descriptionController.text =
          widget.editingProductType!.description ?? '';
      _imageUrl = widget.editingProductType!.thumbnailUrl;
    }

    // Set preselected subcategory if provided
    if (widget.preselectedSubcategoryId != null) {
      _loadSubcategories().then((_) {
        final preselectedSubcategory = _subcategories.firstWhere(
          (sub) => sub.id == widget.preselectedSubcategoryId,
          orElse: () => _subcategories.isNotEmpty
              ? _subcategories.first
              : SubcategoryModel(id: '', categoryId: '', name: ''),
        );
        if (preselectedSubcategory.id.isNotEmpty) {
          setState(() {
            _selectedSubcategory = preselectedSubcategory;
          });
        }
      });
    }
  ...
  Future<void> _saveProductType() async {
    ...
    try {
      ...
      if (_isEditing && _editingProductType != null) {
        // Update existing product type
        final updatedProductType = ProductTypeModel(
          id: _editingProductType!.id,
          ...
          createdAt: _editingProductType!.createdAt,
        );
        await _firestoreService.updateProductType(updatedProductType);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Product type updated successfully!')),
          );
        }
      } else {
        // Create new product type
        final newProductType = ProductTypeModel(
        ...
        await _firestoreService.addProductType(newProductType);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Product type added successfully!')),
          );
        }
      }
      _resetForm();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error saving product type: $e')),
        );
      }
    } finally {
      setState(() => _isLoading = false);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          //   tooltip: 'View All Product Types',
          // ),
        ],
      ),
      body: Container(
          ...
          child: Container(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    _isEditing ? 'Edit Product Type' : 'Add New Product Type',
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton(
                          onPressed: _isLoading ? null : _saveProductType,
                          style: ElevatedButton.styleFrom(
                            backgroundColor: PurviVogueColors.deepNavy,
                            foregroundColor: PurviVogueColors.white,
